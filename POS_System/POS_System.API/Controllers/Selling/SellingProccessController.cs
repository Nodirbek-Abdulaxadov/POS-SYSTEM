...
namespace POS_System.API.Controllers.Identity
{
    ...
    public class SellingProductsController : ControllerBase
    {
        ...
        [HttpPost]
        [Route("add")]
        public async Task<IActionResult> AddSellingProccess(SellingProccess sellingProccess)
        {
            var res = await _sellingProccessInterface.AddSellingProccessAsync(sellingProccess);
            return Ok(res);
